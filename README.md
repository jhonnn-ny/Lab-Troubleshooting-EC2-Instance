...
When you create the instance, you will reference a user data script to configure the instance to have an Apache web server, a MariaDB relational database (which is a fork of the MySQL relational database), and PHP running on the instance. Together, these software packages installed on a single machine are often referred to as a LAMP stack (**L**inux, **A**pache web server, **M**ySQL, and **P**HP). Using a LAMP stack is a common way to create a website with a database backend on a single machine.
...
1. To set up the AWS CLI profile with credentials, run the following command:
...
1. To change to the directory where the script file exists and create a backup of it, run the following commands:
    
    ```
    
    cd ~/sysops-activity-files/starterscp create-lamp-instance-v2.sh create-lamp-instance.backup
    ```
...
1. To display the contents of the user data script, run the following command:
...
1. Now that you have an idea what the shell script is designed to do, try to run it:
...
1. Connect to the new *LAMP instance* by using EC2 Instance Connect, which is the method that you used to connect to the CLI Host instance.
...
1. In the terminal window for the *CLI Host* instance, run the following command to install nmap, which is a port scanning tool:
...
2. 
3. Next, run the following command. Replace *<public-ip>* with the actual public IPv4 address of your *LAMP instance*:
    ...
    ![image.png](attachment:ac1ecef1-7234-4a27-ab2d-5276b4c3f66b:image.png)
    

### **Test whether the user data script ran**

1. After you identify and resolve the issue, in a browser, navigate to the following address. Replace *<public-ip>* with the Public IPv4 address of the new instance that you created: `http://<public-ip>`
    
    If you resolved issue #2 successfully, you should see the following message: "Hello From Your Web Server!"
    
2. Check the log file that shows whether the user data script ran as expected.
    ...
    When you are finished with your observations, to exit the tail utility, enter `Ctrl-C`**Tip:** To view the entire log file, run `sudo cat /var/log/cloud-init-output.log`
    

## **Task 4: Verifying the functionality of the website**

1. Verify that the website is deployed.
...
2. Test whether you can order items through the website.
